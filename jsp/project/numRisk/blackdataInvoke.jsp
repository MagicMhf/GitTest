...
<aos:onready>
	<aos:viewport layout="fit">
		...
		<aos:panel border="false">
			...
			<aos:panel flex="1" layout="fit">
				<aos:gridpanel id="g_thirdele" url="mainService.listBlackCall" onrender="g_thirdele_query">
					<aos:docked forceBoder="1 0 1 0">
	...
	<script type="text/javascript">
        ...
        function download_template() {
            ...
            AOS.file('do.jhtml?router=mainService.downloadBlackTemplate&juid=${juid}');
        }
        //导入文件
		function importFile() {
            if(!f_upload.isValid()) {
            ...
            f_upload.getForm().submit({
                url:'doUpload.jhtml?router=mainService.doUploadBlack&juid=${juid}&ramin_num=${ramin_num}',
                waitMsg:'文件上传中...',
...
function fn_export_excel(id,file_name){
	//juid需要再这个页面的初始化方法中赋值,这里才引用得到
	//httpModel.setAttribute("juid", httpModel.getInDto().getString("juid"));
	AOS.file('do.jhtml?router=mainService.doDownloadBlack&id='+id+'&juid=${juid}');
}
