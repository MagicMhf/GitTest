...
<aos:onready>
	<aos:viewport layout="fit">
		...
		<aos:panel border="false">
			...
			<aos:panel flex="1" layout="fit">
				<aos:gridpanel id="g_thirdele" url="mainService.listMoonCall" onrender="g_thirdele_query">
					<aos:docked forceBoder="1 0 1 0">
						<aos:dockeditem xtype="tbtext" text="月消费调用信息" />
					</aos:docked>
	...
	<script type="text/javascript">
        ...
        function download_template() {
            ...
            AOS.file('do.jhtml?router=mainService.downloadMoonTemplate&juid=${juid}');
        }
        //导入文件
		function importFile() {
            if(!f_upload.isValid()) {
            ...
            f_upload.getForm().submit({
                url:'doUpload.jhtml?router=mainService.doUploadMoon&juid=${juid}&ramin_num=${ramin_num}',//'upload.jhtml?juid=${juid}'
                waitMsg:'文件上传中...',
...
function fn_export_excel(id,file_name){
	AOS.file('do.jhtml?router=mainService.doDownloadMoon&id='+id+'&juid=${juid}');
}
