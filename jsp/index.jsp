...
<aos:html title="${app_title}" base="http" lib="ext,buttons,filter,jquery">
...
.search button.btn {
	...
	margin: 0px auto;
	margin: 1px;
	background: url(${cxt}/static/image/unicom/search_blue.png) no-repeat top right;
}
.search button.btn:hover {
	background: url(${cxt}/static/image/unicom/search_black.png) no-repeat bottom right;
}
</style>
<aos:body>
	<%-- 显示loading --%>
	...
	<div id="div_north_el" class="x-hidden north_el">
		<table style="width: 100%">
			<tr>
				<td width="300px"><img src="${cxt}/static/image/logo/logo-unicom.png"></td>
				<td align="left"><fieldset class="search">
					<input type="text" class="box" name="s" id="s" class="inputText" placeholder="搜   索" x-webkit-speech>
					<button onclick="searchModule()" class="btn"> </button>
				</fieldset></td>
				<td align="right" style="padding: 5px;" width="260px">
					<table style="border-spacing: 3px;">
						...
						<tr>
						<tr align="right">
							<td><a class="${navDto.right_button_style}"
				...
				<%--<aos:docked forceBoder="0 0 1 0">
					...
					<aos:dockeditem text="" tooltip="更多选型" icon="icon141.png">
						<aos:menu>
							...
							&lt;%&ndash; <aos:menuitem text="锁定离开" icon="lock.png" onclick="fn_find_modules" /> &ndash;%&gt;
...
<script type="text/javascript">
	function fnrefreshtab(url, menuname, module_id, root_id){
		var id = "id_tab_" + module_id;
    	var tabs = Ext.getCmp('tabs');
		var tab = tabs.getComponent(id);
		Ext.get(id + '.iframe').dom.src = url;
		tab.setActiveTab(np);
	}

	//打开菜单功能页面
	function fnaddtab(url, menuname, module_id, root_id) {
	    console.log(url+","+menuname+","+module_id+","+root_id);
		if (Ext.isEmpty(url)) {
			return;
		}
		var id = "id_tab_" + module_id;
		...
		if (!tab) {
			...
			document.getElementsByName(id + '.iframe-frame')[0].src=url;
		}
		//激活新增Tab
	...
	function searchModule(){
	    ...
	    //console.log(Ext.get('iframe_main').dom.contentWindow.document.body);
        Ext.get('iframe_main').dom.contentWindow.searchModule(keyName);
	}
</script>